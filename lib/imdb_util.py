...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, conf, root, cache_folder=None):
        ...
        self.use_el = ('use_el' in conf) and conf.use_el
        self.use_el_z = ('use_el_z' in conf) and conf.use_el_z
        self.use_gp = ('use_gp' in conf) and conf.use_gp
        self.gp_max = 30 if not ('gp_max' in conf) else conf.gp_max
        self.restricted_alpha = conf.restricted_alpha
            ...
            for dbind, db in enumerate(conf.datasets_train):
                ...
                if db['anno_fmt'].lower() == 'kitti_det':
                    ...
                    for annind, annpath in enumerate(annlist):
                        ...
                        gts = read_kitti_label(annpath, p2, self.use_3d_for_2d, self.use_el, self.restricted_alpha)
                ...
                # kitti depth information
                elif db['anno_fmt'].lower() == 'kitti_depth':

                    train_folder = os.path.join(root, db['name'], 'train')

                    folders = list_files(train_folder, '/20*')

                    for find, folder in enumerate(folders):

                        _, seq_name, _ = file_parts(folder)

                        # do NOT use sequences in the val or test. whitelist in allowed_seqs
                        if not seq_name in conf.allowed_seqs:
                            #logging.info('{} skipped.'.format(seq_name))
                            continue

                        impaths = list_files(os.path.join(folder, 'image_02'), '/*' + db['im_ext'])
                        impaths += list_files(os.path.join(folder, 'image_03'), '/*' + db['im_ext'])

                        for imind, impath in enumerate(impaths):

                            base, id, ext = file_parts(impath)
                            _, imtype, _ = file_parts(base)

                            # store gts
                            obj = edict()
                            obj.id = id
                            obj.depth_path = os.path.join(folder, 'proj_depth', 'groundtruth', imtype, id + ext)
                            ...
                            if len(imdb_single_db) % 2000 == 0:
                                logging.info('Loaded {} total depth images'.format(len(imdb_single_db)))

                # kitti visual odometry information
                elif db['anno_fmt'].lower() == 'kitti_vo':

                    train_folder = os.path.join(root, db['name'], 'sequences')

                    folders = list_files(train_folder, '/*')

                    for find, folder in enumerate(folders):

                        _, seq_name, _ = file_parts(folder)

                        if not int(seq_name) in conf.vo_train_seqs: continue

                        # read poses
                        poses = read_kitti_poses(os.path.join(root, db['name'], 'poses', seq_name+'.txt'))

                        impaths = list_files(os.path.join(folder, 'image_2'), '/*' + db['im_ext'])

                        for imind in range(1, len(impaths)):

                            impath_pre = impaths[imind-1]
                            impath_cur = impaths[imind]

                            base, id_pre, ext = file_parts(impath_pre)
                            base, id_cur, ext = file_parts(impath_cur)
                            _, imtype, _ = file_parts(base)
                            ...
                            obj.pose_pre = poses[imind-1]
                            obj.pose = poses[imind]

                            # im properties
                            im = Image.open(impath_cur)
                            obj.path = impath_cur
                            obj.path_pre = impath_pre
                            obj.imW, obj.imH = im.size
                        ...
                        logging.info('Loaded seq {}. {} total images'.format(seq_name, len(imdb_single_db)))
                ...
                # kitti with some tracking information
                    ...
                    for annind, annpath in enumerate(annlist):
                        ...
                        all_poses = read_kitti_poses(os.path.join(root, db['name'], 'raw_extra', obj.raw_id[0], 'pose.txt'))
                        ...
                        #  32, 31, 30, 21, 20, 10
                        obj.ego_32 = compute_rel_pose(ego_3, ego_2)
                        obj.ego_31 = compute_rel_pose(ego_3, ego_1)
                        obj.ego_30 = compute_rel_pose(ego_3, ego_0)
                        obj.ego_21 = compute_rel_pose(ego_2, ego_1)
                        obj.ego_20 = compute_rel_pose(ego_2, ego_0)
                        obj.ego_10 = compute_rel_pose(ego_1, ego_0)
                        ...
                        if os.path.exists(os.path.join(extra_path)) and (int(obj.raw_id[1]) - 3) >= 0:
                            ...
                            obj.gts = read_kitti_label(os.path.join(extra_path, '{:06d}.txt'.format(int(obj.raw_id[1]) - 0)), p2, self.use_3d_for_2d)
                            obj.gts_1 = read_kitti_label(os.path.join(extra_path, '{:06d}.txt'.format(int(obj.raw_id[1]) - 1)), p2, self.use_3d_for_2d)
                            obj.gts_2 = read_kitti_label(os.path.join(extra_path, '{:06d}.txt'.format(int(obj.raw_id[1]) - 2)), p2, self.use_3d_for_2d)
                            obj.gts_3 = read_kitti_label(os.path.join(extra_path, '{:06d}.txt'.format(int(obj.raw_id[1]) - 3)), p2, self.use_3d_for_2d)
                ...
                # bbGt v3 formatting (see pdollar toolbox and Caltech pedestrian dataset)
                elif db['anno_fmt'].lower() == 'bbgtv3':

                    train_folder = os.path.join(root, db['name'], 'train')

                    # get sorted filepaths
                    annlist = sorted(glob(os.path.join(train_folder, 'annotations', '') + '*.txt'))
                    ...
                    for annind, annpath in enumerate(annlist):
                        ...
                        impath = os.path.join(train_folder, 'images', id + db['im_ext'])

                        # read gts
                        gts = read_bbGtsv3(annpath)
        ...
        self.len = len(imdb)
        self.imdb = imdb
        self.depth_map = None if 'depth_map' not in conf else conf.depth_map
        ...
        #elif conf.datasets_train[0]['anno_fmt'].lower() == 'kitti_tracking':
        #    self.loader = torch.utils.data.DataLoader(self, conf.batch_size, collate_fn=self.collate, shuffle=True)
    ...
    def __getitem__(self, index):
        ...
        if self.depth_map is not None:
            ...
            base, name, ext = file_parts(self.imdb[index].path.replace('image_2', 'depth_map'))
            depth_path = os.path.join(base, name + self.depth_map)
            imobj.depth_map = pickle_read(depth_path)[:, :, np.newaxis]
        else:
            imobj.depth_map = None
...
def read_bbGtsv3(file):
    """
    Read the dataset according to the Caltech / bbgtv3 data format:
    cls x y w h occ x y w h ign ang

    Args:
        file (str): path for the text file for a single image.
    """
    ...
    # regular expression for following line format:
    # cls x y w h occ x y w h ign ang
    bbGtpat = re.compile(('([a-zA-Z\-\?\_]+)\s+(fpat)\s+(fpat)\s+(fpat)\s+(fpat)\s+(fpat)\s+(fpat)\s+(fpat)\s+'
                       + '(fpat)\s+(fpat)\s+(fpat)\s+(fpat)\s*\n').replace('fpat', '[-+]?\d*\.\d+|[-+]\d+'))

    for line in text_file:

        parsed = bbGtpat.fullmatch(line)
        ...
        if parsed is not None:
            ...
            w = float(parsed.group(4))
            h = float(parsed.group(5))

            obj.bbox_full = np.array([x, y, w, h])

            # occlusion or ignore?
            occ = bool(float(parsed.group(6)))
            ign = bool(float(parsed.group(11)))

            if occ:
                occ_x = float(parsed.group(7))
                occ_y = float(parsed.group(8))
                occ_w = float(parsed.group(9))
                occ_h = float(parsed.group(10))
                obj.bbox_vis = np.array([occ_x, occ_y, occ_w, occ_h])

                # compute visibility
                bbox_full = bbXYWH2Coords(deepcopy(obj.bbox_full)[np.newaxis, :])
                bbox_vis = bbXYWH2Coords(deepcopy(obj.bbox_vis)[np.newaxis, :])
                obj.visibility = iou(bbox_full, bbox_vis)[0, 0]

            else:
                obj.bbox_vis = np.array([x, y, w, h])
                obj.visibility = 1.0
...
def read_kitti_label(file, p2, use_3d_for_2d=False, use_el=False, restricted_alpha=False):
    """
    ...
    for line in text_file:
        ...
        if parsed is not None:
            ...
            if restricted_alpha:
                ...
                while alpha > (math.pi / 2): alpha -= math.pi
                while alpha <= (-math.pi / 2): alpha += math.pi
            ...
            if use_el:
                obj.bbox_3d = [cx, cy, elevation, w3d, h3d, l3d, alpha, cx3d, cy3d, cz3d, rotY, elevation, alpha_sin, alpha_cos, axis_lbl, head_lbl]
            else:
