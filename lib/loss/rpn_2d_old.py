...
class RPN_2D_loss(nn.Module):
    ...
    def __init__(self, num_classes, anchors, bbox_means, bbox_stds, feat_stride, fg_fraction, box_samples, ign_thresh,
                 ...
                 moving_target, lbls, ilbls, min_gt_vis, min_gt_h, max_gt_h, cls_2d_lambda, iou_2d_lambda,
                 bbox_2d_lambda, gpu=0):

        super(RPN_2D_loss, self).__init__()
        ...
        self.cls_2d_lambda = cls_2d_lambda
        self.iou_2d_lambda = iou_2d_lambda
        self.bbox_2d_lambda = bbox_2d_lambda
    ...
    def forward(self, cls, prob, bbox_2d, imobjs):
        ...
        for bind in range(0, batch_size):
            ...
            if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
                ...
                # bbox regression
                transforms, ols = compute_targets(gts_val, gts_ign, box_lbls, rois.numpy(), self.fg_thresh, self.ign_thresh,
                                             self.bg_thresh_lo, self.bg_thresh_hi, self.best_thresh,
                ...
                if fg_num > 0:
                    ...
                    if self.moving_target:
                        ...
                        found = gt_best_ols >= self.best_thresh
                        ...
                        if best_inds.shape[0] > 0:
                            ...
                            # recompute fg / bg samples
                            fg_inds = np.flatnonzero((labels[bind, :] > 0) & (labels[bind, :] != IGN_FLAG))
                            bg_inds = np.flatnonzero(labels[bind, :] == 0)
        ...
        if np.sum(bbox_weights) > 0:
            ...
            stats.append({'name': 'iou', 'val': coords_2d_iou[active].mean(), 'format': '{:0.2f}', 'group': 'acc'})
