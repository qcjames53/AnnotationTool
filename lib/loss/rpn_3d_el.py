...
class RPN_3D_El_loss(nn.Module):

    def __init__(self, conf):

        super(RPN_3D_El_loss, self).__init__()
    ...
    def forward(self, cls, prob, bbox_2d, bbox_3d, imobjs, feat_size):
        ...
        coords_cen_dist = torch.zeros(cls.shape[0:2])
        ...
        bbox_elv_dn = bbox_elv * self.bbox_stds[:, 6][0] + self.bbox_means[:, 6][0]
        ...
        bbox_elv_dn = src_anchors[:, 4].unsqueeze(0) + bbox_elv_dn
        ...
        for bind in range(0, batch_size):
            ...
            if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
                ...
                bbox_elv_tar[bind, :] = transforms[:, 7]
                ...
                if fg_num > 0:
                    ...
                    bbox_elv_dn_fg = bbox_elv_dn[bind, fg_inds]
                    ...
                    y3d = bbox_elv_dn_fg  - bbox_h3d_dn_fg * 0.5
                    z3d = (y3d*p2_d + p2_f - p2_h*bbox_y3d_dn_fg) / (bbox_y3d_dn_fg - p2_e)
                    # suppress avoid horizon errors
                    z3d = z3d.clamp(min=-5, max=65)
                    ...
                    # absolute targets
                    bbox_z3d_dn_tar = bbox_elv_tar[bind, fg_inds] * self.bbox_stds[:, 6][0] + self.bbox_means[:, 6][0]
                    ...
                    #coords_abs_z[bind, fg_inds] = torch.abs(bbox_z3d_dn_tar - bbox_elv_dn_fg)
        ...
        if np.sum(bbox_weights) > 0:
            ...
            cen_dist = torch.sqrt(((bbox_x3d_proj[active] - bbox_x3d_proj_tar[active])**2)
                                  ...
                                  + ((bbox_z3d_proj[active] - bbox_z3d_proj_tar[active])**2)).mean()
            ...
            if self.bbox_3d_lambda:
                ...
                loss_bbox_z3d = F.smooth_l1_loss(bbox_elv[active], bbox_elv_tar[active], reduction='none') / closeness
            ...
            stats.append({'name': 'cen', 'val': cen_dist.detach(), 'format': '{:0.2f}', 'group': 'misc'})
