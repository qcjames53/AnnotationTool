...
class RPN_3D_Plane_loss(nn.Module):

    def __init__(self, conf):

        super(RPN_3D_Plane_loss, self).__init__()
    ...
    def forward(self, cls, prob, bbox_2d, bbox_3d, plane_affine, imobjs, feat_size):
        ...
        bbox_x3d_dn = torch.zeros(cls.shape[0:2])
        bbox_y3d_dn = torch.zeros(cls.shape[0:2])
        bbox_z3d_dn = torch.zeros(cls.shape[0:2])
        ...
        for bind in range(0, batch_size):
            ...
            if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
                ...
                transforms_dn = torch.from_numpy(transforms_dn).cuda()
                ...
                bbox_x3d_tar[bind, :] = transforms_dn[:, 5]
                bbox_y3d_tar[bind, :] = transforms_dn[:, 6]
                bbox_z3d_tar[bind, :] = transforms_dn[:, 7]
                ...
                if fg_num > 0:
                    ...
                    coords_2d_np = coords_2d_tar[fg_inds, :].detach().cpu().numpy() / imobj['scale_factor']
                    _, coords_3d_def = projection_ray_trace(imobj.p2, imobj.p2_inv, (coords_2d_np[:, 0] + coords_2d_np[:, 2]) / 2, coords_2d_np[:, 3], 1.65)

                    coords_3d_def = torch.from_numpy(coords_3d_def[:plane_affine.shape[1], :]).type(torch.cuda.FloatTensor)

                    coords_warped = torch.mm(plane_affine[bind,:,:], coords_3d_def)

                    # replace predictions using the warped!
                    bbox_x3d_dn[bind, fg_inds] = coords_warped[0, :]
                    bbox_y3d_dn[bind, fg_inds] = coords_warped[1, :]
                    bbox_z3d_dn[bind, fg_inds] = coords_warped[2, :]
        ...
        if np.sum(bbox_weights) > 0:
            ...
            if self.bbox_3d_lambda:
                ...
                bbox_x3d_dn = bbox_x3d_dn[:, :].view(-1)
                bbox_y3d_dn = bbox_y3d_dn[:, :].view(-1)
                bbox_z3d_dn = bbox_z3d_dn[:, :].view(-1)
                ...
                tmp = 0.000
                loss_bbox_x3d = F.smooth_l1_loss(bbox_x3d_dn[active], bbox_x3d_tar[active], reduction='none')*tmp
                loss_bbox_y3d = F.smooth_l1_loss(bbox_y3d_dn[active], bbox_y3d_tar[active], reduction='none')*tmp
                loss_bbox_z3d = F.smooth_l1_loss(bbox_z3d_dn[active], bbox_z3d_tar[active], reduction='none')*tmp
