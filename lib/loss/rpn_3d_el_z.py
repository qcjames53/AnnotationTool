...
class RPN_3D_El_z_loss(nn.Module):

    def __init__(self, conf):

        super(RPN_3D_El_z_loss, self).__init__()
        ...
        self.elv_lambda = conf.elv_lambda
    ...
    def forward(self, cls, prob, bbox_2d, bbox_3d, imobjs, feat_size):
        ...
        bbox_elv_tar = np.zeros(cls.shape[0:2])
        ...
        for bind in range(0, batch_size):
            ...
            if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
                ...
                raw_gt = torch.from_numpy(raw_gt).cuda()
                ...
                if fg_num > 0:
                    ...
                    bbox_h3d_dn_fg = bbox_h3d_dn[bind, fg_inds].detach()
                    bbox_l3d_dn_fg = bbox_l3d_dn[bind, fg_inds]
                    bbox_ry3d_dn_fg = bbox_ry3d_dn[bind, fg_inds]
                    bbox_elv_dn_fg = bbox_elv[bind, fg_inds]

                    # re-scale all 2D back to original
                    bbox_x3d_dn_fg /= imobj['scale_factor']
                    bbox_y3d_dn_fg /= imobj['scale_factor']

                    # compute 3D location using direct z
                    z3d_direct = bbox_z3d_dn_fg - p2_h

                    # compute 3D location using elv
                    #y3d_elv = torch.from_numpy(bbox_elv_tar[bind, fg_inds]).type(torch.cuda.FloatTensor) - bbox_h3d_dn_fg * 0.5
                    y3d_elv = bbox_elv_dn_fg - bbox_h3d_dn_fg * 0.5
                    z3d_elv = (y3d_elv * p2_d + p2_f - p2_h * bbox_y3d_dn_fg) / (bbox_y3d_dn_fg - p2_e)
                    z3d_elv = z3d_elv.clamp(min=1, max=65)
                    x3d_elv = ((z3d_elv + p2_h) * bbox_x3d_dn_fg - p2_b * (z3d_elv) - p2_c) / p2_a

                    bbox_x3d_proj[bind, fg_inds] = x3d_elv
                    bbox_y3d_proj[bind, fg_inds] = y3d_elv
                    bbox_z3d_proj[bind, fg_inds] = z3d_elv

                    bbox_elv_proj[bind, fg_inds] = (z3d_elv + p2_h)
                    bbox_dir_proj[bind, fg_inds] = (z3d_direct + p2_h)
                    ...
                    coords_abs_z[bind, fg_inds] = torch.abs(bbox_z3d_proj_tar[bind, fg_inds] - bbox_z3d_proj[bind, fg_inds])
        ...
        if np.sum(bbox_weights) > 0:
            ...
            if self.bbox_2d_lambda:
                ...
                loss += bbox_2d_loss
                stats.append({'name': 'bbox_2d', 'val': bbox_2d_loss.detach(), 'format': '{:0.4f}', 'group': 'loss'})
            ...
            bbox_elv_proj = bbox_elv_proj[:, :].view(-1)
            ...
            stats.append({'name': 'cen', 'val': cen_dist.mean().detach(), 'format': '{:0.2f}', 'group': 'misc'})
            ...
            if self.bbox_3d_lambda:
                ...
                #bbox_elv_tar = torch.tensor(bbox_elv_tar, requires_grad=False).type(torch.FloatTensor).cuda().view(-1)
                ...
                loss_bbox_elv = self.elv_lambda*F.smooth_l1_loss(bbox_elv_proj[active], bbox_z3d_tar[active], reduction='none') / closeness

                if self.all_un:
                    loss_bbox_x3d = (loss_bbox_x3d * bbox_weights[active]) * bbox_un[active]
                    loss_bbox_y3d = (loss_bbox_y3d * bbox_weights[active]) * bbox_un[active]
                    loss_bbox_z3d = (loss_bbox_z3d * bbox_weights[active]) * bbox_un[active]
                    loss_bbox_w3d = (loss_bbox_w3d * bbox_weights[active]) * bbox_un[active]
                    loss_bbox_h3d = (loss_bbox_h3d * bbox_weights[active]) * bbox_un[active]
                    loss_bbox_l3d = (loss_bbox_l3d * bbox_weights[active]) * bbox_un[active]
                    loss_bbox_ry3d = (loss_bbox_ry3d * bbox_weights[active]) * bbox_un[active]
                    loss_bbox_elv = (loss_bbox_elv * bbox_weights[active]) * bbox_un[active]
                else:
                    ...
                    loss_bbox_elv = (loss_bbox_elv * bbox_weights[active])
                ...
                bbox_3d_loss = (loss_bbox_w3d[torch.isfinite(loss_bbox_w3d)].mean()
                                ...
                                + loss_bbox_elv[torch.isfinite(loss_bbox_elv)].mean())
            ...
            if self.iou_2d_lambda:
                ...
                stats.append({'name': 'iou', 'val': iou_2d_loss.detach(), 'format': '{:0.4f}', 'group': 'loss'})
